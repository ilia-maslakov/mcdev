/*
   Kubernetes panel plugin -- kubectl helpers, config, cache, favorites.
...
k8s_load_hotkey (const char *key, const char *fallback_text, int fallback_key)
{
    ...
    {
        g_free (value);
        return (value != NULL && g_ascii_strcasecmp (value, "none") == 0) ? 0 : fallback_key;
    }
...
k8s_favorites_load (void)
{
    ...
    list = mc_config_get_string_list (cfg, K8S_FAVORITES_CONFIG_GROUP, K8S_FAVORITES_CONFIG_KEY,
                                      &len);
    if (list != NULL)
    {
        for (i = 0; i < len; i++)
...
void
k8s_cache_entry_free (gpointer p)
{
    k8s_cache_entry_t *entry = (k8s_cache_entry_t *) p;

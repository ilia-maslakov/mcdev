...
/* Reverse key definitions.  define_sequence() is the single place where MC
 * learns sequence -> key, so keep key -> last learned sequence in sync there. */
...
#ifdef HAVE_NCURSES
    /* ncurses pre-encodes modifier+Fn as extended KEY_F(n) codes with no
     * modifier bits set.  Remap to MC's convention (modifier | KEY_F(1..12))
     * before the generic Shift+Fn conversion below, which only handles keys
     * that already carry KEY_M_SHIFT.
     *   kf13..kf24 (KEY_F(13..24)) = Shift+F1..F12
     *   kf25..kf36 (KEY_F(25..36)) = Ctrl+F1..F12
     *   kf49..kf60 (KEY_F(49..60)) = Alt+F1..F12 */
...
static int
normalize_defined_key_code (int code)
{
    return tty_normalize_keycode (code);
}
...
remember_key_sequence (int code, const char *seq)
{
    ...
    normalized_code = normalize_defined_key_code (code);
...
tty_key_lookup_sequence (int code)
{
    ...
    seq = key_sequences != NULL
        ? g_hash_table_lookup (key_sequences, GINT_TO_POINTER (normalize_defined_key_code (code)))
        : NULL;
    if (seq != NULL)

...
AC_DEFUN([mc_G_MODULE_SUPPORTED], [
    ...
    found_gmodule=no
    PKG_CHECK_MODULES(GMODULE, [gmodule-no-export-2.0 >= 2.58], [found_gmodule=yes], [:])
    if test x"$found_gmodule" = xyes; then
        g_module_supported="gmodule-no-export-2.0"
    else
        dnl try fallback to the generic gmodule
        PKG_CHECK_MODULES(GMODULE, [gmodule-2.0 >= 2.58], [found_gmodule=yes], [:])
        if test x"$found_gmodule" = xyes; then
...
AC_DEFUN([mc_CHECK_GLIB], [
    ...
    glib_found=no
    PKG_CHECK_MODULES(GLIB, [glib-2.0 >= 2.58], [glib_found=yes], [:])
    if test x"$glib_found" = xno; then
        AC_MSG_ERROR([glib-2.0 not found or version too old (must be >= 2.58)])
    fi
